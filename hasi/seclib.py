...
def get_credentials(key):
    ...
    # File was read, but no corresponding information for given key was found.
    log.error("The connection information Key {0} could not be found in credential file.".format(key))
    return False
...
def encrypt_cbc(key, s_data, codepage):
    from Crypto.Cipher import AES
    from Crypto.Util.Padding import pad
...
def decrypt_cbc(key, s_data, codepage):
    from Crypto.Cipher import AES
