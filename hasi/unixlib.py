...
unixlib_log = logging.getLogger("unixlib")
unixlib_log.setLevel(logging.DEBUG)
formatter = logging.Formatter("%(asctime)s - %(name)s - %(levelname)s - %(message)s")
console_handler = logging.StreamHandler()
console_handler.setLevel(logging.INFO)
console_handler.setFormatter(formatter)
unixlib_log.addHandler(console_handler)
...
def scp_remote_files(host, user, path, filename, local_path="."):
    ...
    if stmt.returncode != 0 and "No such file or directory" in error.decode(standard_encoding).strip():
        ...
        print(output.decode(standard_encoding).strip())
        return True
    else:
        return True
...
def remote_shell(host, user, command, password=None):
    ...
    try:
        ...
        print(stdout.read().decode(standard_encoding))
        v_event_code = stdout.channel.recv_exit_status()
        if stdout.channel.recv_exit_status() != "0":
            print(stderr.read().decode(standard_encoding))
        ssh.close()
        return v_event_code
...
def convert_date(x, date_format, lazy=False):
    ...
    if lazy is True:
        ...
        unixlib_log.debug("{0}-{1}-{2}".format(year, month, day))
        try:
...
def convert_time(x, time_format, lazy=False):
    ...
    if lazy is True:
        ...
        unixlib_log.debug("{0}:{1}:{2}".format(hour, minute, second))
        try:
...
def convert_timestamp(x, timestamp_format, lazy=False):
    ...
    if lazy is True:
        ...
        minute = "00" if minute > "59" else minute
        second = "00" if second > "59" else second
        unixlib_log.debug("{0}-{1}-{2} {3}:{4}:{5}".format(year, month, day, hour, minute, second))
        try:
...
class FileObject(object):
    ...
    def get_column_list(self):
        ...
        if set_result is False:
            unixlib_log.info("Table {0} was not found in metadata table obj_ddic_catalog for SID {1}".format(
    ...
    def get_avro_schema(self):
        ...
        for item in l_descriptor:
            ...
            unixlib_log.debug(d_field_entry)
        ...
        unixlib_log.debug("AVRO Schema: {0}".format(self.avro_schema))
        return self.avro_schema

    def csv_export(self, directory, filename, extraction_timestamp, headline_exists,
                   metadata_columns=None, lazy_read=False, suppress_errors=False):
        """
        ...
        for i in metadata_columns:
            unixlib_log.debug("Metadatenfeld: {0}".format(i))
            if i == "TA_CHDATE":
        ...
        unixlib_log.debug("Kopfzeile: {0}".format(headline))
        ...
        unixlib_log.info("Export {0}, file: {1}\nto CSV file {2}\nCodepage: {3}, \nDelimiter: {4}.".format(
    ...
    def avro2csv(directory, filename, delimiter="|"):
        ...
        with open("{0}/{1}".format(directory, filename), "rb") as s:
            with open("{0}/{1}.csv".format(directory, filename), "w") as t:
                ...
                unixlib_log.debug("Schema: {0}".format(extracted_schema))
