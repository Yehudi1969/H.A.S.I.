...
if __name__ == "__main__":
    ...
    options.add_argument("-d", "--delimiter", dest="delimiter", help="Trennzeichen, default=|", default="|")
    ...
    with open(filename, "w") as fh:
        if filename == "lu_datum_org.csv":
            headline = delimiter.join_cond(list(str(x).upper() for x in l_hd_tag))
            l_ersatzwerte = ['-1', '9999-12-31 23:59:59', '0', '-1', '-1', '-1', '-1', 'n.r.',
                             '0', 'n.r.', '-', '-', '0', '-', '-', '-1', '-1', '-1', '-1', '-1', '9999999999', '999999']
            add_column = delimiter.join_cond(list(str(x).upper() for x in l_ersatzwerte))
        elif filename == "lu_halbjahr_org.csv":
            headline = delimiter.join_cond(list(str(x).upper() for x in l_hd_halbjahr))
            l_ersatzwerte = ['-1', '-1', 'n.r.', '0', 'n.r.', '9999999999', '999999']
            add_column = delimiter.join_cond(list(str(x).upper() for x in l_ersatzwerte))
        elif filename == "lu_jahr_org.csv":
            headline = delimiter.join_cond(list(str(x).upper() for x in l_hd_jahr))
            l_ersatzwerte = ['-1', '0', '9999999999', '999999']
            add_column = delimiter.join_cond(list(str(x).upper() for x in l_ersatzwerte))
        elif filename == "lu_monat_org.csv":
            headline = delimiter.join_cond(list(str(x).upper() for x in l_hd_monat))
            l_ersatzwerte = ['-1', '-1', 'n.r.', '0', 'n.r.', '9999999999', '999999']
            add_column = delimiter.join_cond(list(str(x).upper() for x in l_ersatzwerte))
        elif filename == "lu_quartal_org.csv":
            headline = delimiter.join_cond(list(str(x).upper() for x in l_hd_quartal))
            l_ersatzwerte = ['-1', '-1', 'n.r.', '0', 'n.r.', '9999999999', '999999']
            add_column = delimiter.join_cond(list(str(x).upper() for x in l_ersatzwerte))
        elif filename == "lu_woche_org.csv":
            headline = delimiter.join_cond(list(str(x).upper() for x in l_hd_woche))
            l_ersatzwerte = ['-1', 'n.r.', '0', 'n.r.', '9999999999', '999999']
            add_column = delimiter.join_cond(list(str(x).upper() for x in l_ersatzwerte))
        elif filename == "obj_date.csv":
            headline = delimiter.join_cond(list(str(x).upper() for x in l_obj_date))
            add_column = None
        else:
