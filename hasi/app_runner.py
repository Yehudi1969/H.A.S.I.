...
log = logging.getLogger("app_runner")
...
class AppRunner(object):
    def __init__(self, app, b_force=False):
        self.conn = DSN
        self.app = app
        self.force = b_force
        if self.force is True:
            query = "update obj_job_plan set status=0, status_text='PLANNED' where app_name='{0}'".format(self.app)
    ...
    def next_action(self):
        ...
        if len(self.l_remain) > 0:
            ...
            with Pool(processes=len(l_start)) as p:
...
if __name__ == "__main__":
    ...
    obj_app = AppRunner(app_name, b_force=force)
    i_runs = 0
