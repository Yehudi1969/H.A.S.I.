...
# Change history
# v0.1: 2019-04-23 - Initial Release
# v0.2: 2020-07-18 - Multiple Backends implemented
...
class HasiApplication(object):
    ...
    def __init__(self, v_app_name):
        ...
        self.check_app_status('HASI', -9010) if len(result) == 0 else None
        self.environment = db.sql_query(DSN, query)[0][0]
        ...
        self.check_app_status('HASI', -9000) if db.sql_query(DSN, query)[0][0] != 1 else None
...
class HasiProject(HasiApplication):
    ...
    def __init__(self, v_app_name):
        ...
        result = db.sql_query(DSN, query)
        self.check_app_status('HASI', -9026) if len(result) == 0 else None
        ...
        result = db.sql_query(DSN, query)
        self.check_app_status('HASI', -9027) if len(result) == 0 else None
        ...
        query = "select value_text from obj_att_project where project = '{0}' and key_text = 'SRCDIR';".format(
            self.project_name)
        result = db.sql_query(DSN, query)
        self.check_app_status('HASI', -9028) if len(result) == 0 else None
        ...
        query = "select value_text from obj_att_project where project = '{0}' and key_text = 'TGTDIR';".format(
            self.project_name)
        result = db.sql_query(DSN, query)
        self.check_app_status('HASI', -9029) if len(result) == 0 else None
        ...
        query = "select value_text from obj_att_project where project = '{0}' and key_text = 'LOGDIR';".format(
            self.project_name)
        result = db.sql_query(DSN, query)
        self.check_app_status('HASI', -9030) if len(result) == 0 else None
        ...
        if result == 0:
            ...
            query = "SELECT value_text FROM obj_att_project WHERE project = 'ALL' AND key_text = 'PLAN_DATE';"
            self.plan_date = db.sql_query(DSN, query)[0][0]
    ...
    def log_application(self, status):
        ...
        if status == 1:
            ...
            query = "select app_id, to_char(ts_start, 'HH24:MI:SS'), current_timestamp, " \
                    "to_char(current_timestamp, 'HH24:MI:SS') " \
                    "from obj_log_application where app_name = '{0}' and status = '1';".format(self.app_name)
            result = db.sql_query(DSN, query)
            self.app_logger.debug(result)
            self.app_id = result[0][0]
            time_start = result[0][1]
            ts_end = result[0][2]
            time_end = result[0][3]
            time_format = "%H:%M:%S"
            duration = datetime.datetime.strptime(time_end, time_format) - \
                datetime.datetime.strptime(time_start, time_format)
            if duration.days < 0:
                duration = datetime.timedelta(days=0, seconds=duration.seconds, microseconds=duration.microseconds)
            seconds_taken = duration.total_seconds()
...
class HasiSession(HasiProject):
    ...
    def __init__(self, v_app_name, v_job_name):
        ...
        result = db.sql_query(DSN, query)
        self.check_app_status('HASI', -9000) if result is None else None
        ...
        query = "select count(1) from obj_job_plan where app_name = '{0}';".format(self.app_name)
        jobnet_exists = db.sql_query(DSN, query)[0][0]
        if jobnet_exists == 0:
        ...
        """ Check valid job """
        query = "select count(1) from obj_job_plan where app_name = '{0}' and job_name = '{1}';".format(
            self.app_name, self.job_name)
        if db.sql_query(DSN, query)[0][0] != 1:
    ...
    def check_event_status(self, event_class, event_code):
        ...
        if not self.session_logger:
            self.get_session_logger()
        query = "select count(*) from obj_events where event_class = '{0}' " \
                "and event_code = '{1}';".format(event_class, event_code)
        result = db.sql_query(DSN, query)[0][0]
        """ Handler for unknown event code """
        if result == 0:
            ...
            event_code = "1"
            event_action = "ERROR"
            self.check_session_status(event_code, event_action, event_msg)
        query = "select event_msg, event_action from obj_events where event_class = '{0}' " \
                "and event_code = '{1}';".format(event_class, event_code)
        result = db.sql_query(DSN, query)
        self.session_logger.debug(result)
        event_msg = result[0][0] if not event_msg else event_msg
        event_action = result[0][1]
        self.session_logger.debug("Event Message: {0}".format(event_msg))
        self.session_logger.debug("Event Action: {0}".format(event_action))
        self.check_session_status(event_code, event_action, event_msg)

    def check_session_status(self, event_code, event_action, event_msg):
        ...
        if event_action == "":
            event_action = "ERROR"
        if event_action == "SUCCESS":
            self.session_logger.info(event_msg)
            ...
            self.session_logger.warning(event_msg)
            self.session_logger.warning("Session {0} ended with returncode {1}. Process will continue.".format(
                self.job_name, event_code))
            self.update_job_status(2)
            self.log_obj_sess_detail()
            self.end_session(0)
        elif event_action == "ERROR":
            self.session_logger.error(event_msg)
            self.session_logger.error("Session {0}, program {1} aborted with returncode {2}.".format(
                self.job_name, self.program_name, event_code))
            self.update_job_status(3)
            self.log_obj_sess_detail()
            self.send_mail_report(event_code)
            self.end_session(6)
        elif event_action == "ABORT":
            self.session_logger.error(event_msg)
            self.session_logger.error("Session {0}, program {1} aborted with returncode {2}.".format(
                self.job_name, self.program_name, event_code))
            self.send_mail_report(event_code)
    ...
    def log_session(self, status):
        ...
        if status == 1:
            ...
            query = "select job_id, to_char(ts_start, 'HH24:MI:SS'), current_timestamp, " \
                    "to_char(current_timestamp, 'HH24:MI:SS') from obj_log_session where app_id = '{0}' " \
            ...
            self.end_session(1) if len(result) == 0 else None
            ...
            if duration.days < 0:
                duration = datetime.timedelta(days=0, seconds=duration.seconds, microseconds=duration.microseconds)
            seconds_taken = duration.total_seconds()
            query = "update obj_log_session set plan_date = to_date('{0}', 'YYYY-MM-DD'), status = '{1}'" \
                    ", status_text = '{2}', ts_end = to_timestamp('{3}', 'YYYY-MM-DD HH24:MI:SS.FF6')" \
                    ", job_duration = '{4}', seconds_taken = {5}, rows_written = {6}" \
                    ", ts_upd = to_timestamp('{7}', 'YYYY-MM-DD HH24:MI:SS.FF6') " \
                    "where app_id = {8} and job_id = {9};".format(
                     self.plan_date, self.job_status, self.job_status_text, ts_end, duration,
    ...
    def log_obj_sess_detail(self):
        ...
        """
        session_logfile_name = "{0}/{1}.{2}.{3}.{4}.log".format(self.log_directory, self.subproject_name, self.job_type,
                                                                self.program_string, os.getpid())
    ...
    def generate_jobnet(self):
        ...
        if result is False:
            self.session_logger.warning("No jobnet found to delete for application {0}.".format(self.app_name))
    ...
    def run_session(self):
        ...
        if self.job_type == "SET_PLAN_DATE":
            ...
            if file_found is False:
                event_code = 127
                self.check_event_status("UNIX_PROGRAM", event_code)
            ...
            result = db.sql_query(DSN, query)
            self.check_app_status('HASI', -9031) if len(result) == 0 else None
            ...
            result = db.sql_query(DSN, query)
            self.check_app_status('HASI', -9033) if len(result) == 0 else None
    ...
    def signature_check(self):
        ...
        query = "select count(1) from obj_signatures where job_name = '{0}' and obj_name = '{1}';".format(
            self.job_name, self.program_name)
        c_exists = db.sql_query(DSN, query)[0][0]
        if c_exists == 0:
            """ Insert hash """
    ...
    def send_mail_report(self, status):
        ...
        """
        subject = None
        self.session_logger.debug("Datentyp Status: {0}".format(type(status)))
        if status == 0:
            ...
            subject = "ERROR: Job {0} failed in application {1}, Project {2} and Subproject {3} " \
                      "with error code {4}".format(self.job_name, self.app_name,
                                                   self.project_name, self.subproject_name, status)
        ...
        plot_filename = HasiApplication.plot_app(self, self.log_directory, "png")
        ...
        query = "select count(*) from obj_customer_email where project = '{0}' and subproject = '{1}' " \
                "and active = 1;".format(self.project_name, self.subproject_name)
        num_of_recipients = db.sql_query(DSN, query)[0][0]
        if num_of_recipients > 0:
            query = "select email from obj_customer_email where project = '{0}' and subproject = '{1}' " \
                    "and active = 1;".format(self.project_name, self.subproject_name)
            result = db.sql_query(DSN, query)
            self.list_customers = list(x[0] for x in result)

        """ Set customers for alerting in case of error only """
        if status != 0:
            query = "select count(*) from obj_customer_email where project = '{0}' and subproject = '{1}' " \
                    "and active = 0 and alerting = 1;".format(self.project_name, self.subproject_name)
            result = db.sql_query(DSN, query)[0][0]
            self.session_logger.debug(result)
            if result > 0:
                num_of_recipients = num_of_recipients + result
                query = "select email from obj_customer_email where project = '{0}' and subproject = '{1}' " \
                        "and active = 0 and alerting = 1;".format(self.project_name, self.subproject_name)
                result = db.sql_query(DSN, query)
                self.list_customers.extend(list(x[0] for x in result))
                self.session_logger.debug("Alarmmeldungen an: {0}".format(self.list_customers))

        """ Abort sending mail if no recipient has been found """
        self.session_logger.debug("Number of recipients: {0}".format(num_of_recipients))
        if num_of_recipients == 0:
            ...
            if status == 0:
                ...
                self.check_event_status("HASI", -9008)
